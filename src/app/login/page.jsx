...
export default function Page() {
 ...
 return (
    <div className='absolute bottom-0 right-0 top-0 left-0 bg-black text-white h-screen w-full flex justify-center items-center z-20'>
      ...
      <div className='w-[500px] h-1/2 bg-[#1b1b1b] rounded-lg flex flex-col'>
        <div className='flex justify-between items-center bg-violet-500 rounded-t-lg px-6 py-3 '>
          <div className='text-[25px] font-bold'>{'log in'}</div>
          <div className='text-[25px] font-bold w-[100px] rounded-lg h-[50px] bg-black'></div>
        </div>
        <form className='w-full h-full flex flex-col gap-2 p-6 '>
          ...
          {!logged && <button type='submit' onClick={newAccount}>Create account</button>}
        </form>
      </div>
    </div>
 );
