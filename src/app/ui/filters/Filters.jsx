      ...
      <div className={`${openFilter ? 'visible' : 'hidden'} absolute top-[-250px] flex flex-col gap-2 rounded-lg transition-all bg-[#0f0f0faf] backdrop-blur-lg`}>
        ...
        <div className='bg-black font-bold rounded-t-lg py-2 px-4 border-b-[1px] border-[#f2f2f231]'>Rating</div>
          <div className='flex w-full gap-2 text-left'>
            {ratingFilterButtons.map((element, key)=>{return( <button key={key} onClick={()=> dispatch(setRateFilter(element))} 
            className={`${rateFilter != element ? 'text-white' : 'text-violet-500'} transition-all w-1/3 my-2 capitalize`}>{element}</button> )})}
          </div>
        </div>
        <div className='max-h-[150px] overflow-scroll text-left bg-[#0f0f0faf] w-[300px] rounded-lg border-[1px] border-[#f2f2f231] font-light'>
